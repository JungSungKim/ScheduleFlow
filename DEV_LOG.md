...
**모달 외부 클릭·ESC 닫기 비활성화**
- `app.js` — `modal-overlay` 바깥 클릭 리스너 제거 (line 382-384)
- `app.js` — ESC 키에서 `closeModal()` 제거 (검색창 닫기는 유지)
- 모달은 ✕ 버튼 또는 취소 버튼으로만 닫힘
- `app.js` v21→v22, `CACHE_VER` sf-v29→sf-v30, Firebase Hosting 배포 완료

**CLAUDE.md 개선**
- 캐시 버스팅 3단계를 분리 — 기존 2번 항목이 `CACHE_VER 증가 + PRECACHE 동기화`를 묶어 누락 위험 있었음
- 모달 닫기 정책 gotcha 추가
...
- [ ] `public/index.html.tmp.*` 임시 파일 삭제
